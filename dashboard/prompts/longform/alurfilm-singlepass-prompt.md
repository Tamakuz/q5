...
- **KHUSUS PART INTRO (PART #0 / INTRO TEASER HIGHLIGHT - LANGSUNG KE INTI, SEMANGAT & NON-CRINGE)**:
  - **DILARANG BERTELE-TELE & DILARANG FRASA PEMBUKA REPETITIF**: Sapaan pembuka WAJIB singkat, padat, dan langsung menyengat ke konflik utama film! DILARANG KERAS bergantung pada frasa pembuka Paul seperti *"bayangin aja"*. Gunakan variasi sudut pandang pembuka yang dinamis.
  - **Menyebutkan Genre & Judul Film Secara Alami & Wajar**: Sebutkan genre atau judul film ("{{movie_title}}") secara menyatu dan mengalir alami tanpa tanda titik dua kaku, tanpa kata placeholder, DAN tanpa kata pemanis berlebihan (tanpa kata "super epik", "super gokil", dll.). Jika judul "Tidak disebutkan", sebutkan genrenya saja secara wajar.
  - **WAJIB MEMBANGUN CURIOSITY GAP (TEKA-TEKI PENASARAN & PUNCHY HOOK)**:
    * Setelah menyapa dengan semangat, kalimat berikutnya WAJIB langsung melempar pancingan (hook) tentang konflik paling gila, ekstrem, atau misterius di film ini tanpa membocorkan endingnya. Bikin penonton penasaran setengah mati. Gunakan diksi tajam, berenergi, dan bervariasi di setiap naskah.
    * DILARANG MENCERITAKAN KRONOLOGI DI INTRO. Intro HANYA untuk nge-tease momen paling epik/menegangkan.
  - **Emosi High-Energy & Semangat**: Nada emosi WAJIB penuh semangat membara dan antusias tinggi (*high-energy hype narration*)! DILARANG KERAS tone loyo, sedih, atau desahan lesu (`[sigh]` dilarang untuk adegan aksi/hype).
  - **Tanda Baca Sinematik Modulasi TTS**: Gunakan kombinasi tanda baca penegas nada emosi `!`, `?!`, `?`, `—`, `...` untuk memancing intonasi vokal TTS agar naik-turun secara dramatis dan antusias.
  - **Transisi Antar-Kalimat Smooth**: Sambungkan perpindahan antar-kalimat secara halus, organis, dan mengalir mulus (*smooth rhythmic flow*).
  ...
  - **DILARANG TAG VISUAL ONLY DI PART INTRO**: Khusus Part #0 (Intro), DILARANG KERAS menyisipkan tag `[VISUAL_ONLY: ...]`. Naskah voiceover Intro WAJIB mengalir terus-remenerus 100% dari awal sampai akhir tanpa ada jeda/break visual pause!
  - **BEBAS TARGET KATA DI PART INTRO**: Khusus Part #0 (Intro), DILARANG membatasi target kata. Biarkan narasi Intro mengalir organik sesuai cuplikan video teaser tanpa dibatasi jumlah kata kaku!
