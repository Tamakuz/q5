...
const DEFAULT_SCENE_CONTEXT = `Educational History Storytelling for "Waku" YouTube channel (Indonesian audience, 18-35 age group). Semi-formal, engaging, mind-blowing counterintuitive history facts.`;
...
const WakuVoiceOverStep: React.FC<VannVoiceOverStepProps> = ({ onStepChange }) => {
  ...
  useEffect(() => {
    if (api?.onWakuFasterWhisperProgress) {
      const cleanup = api.onWakuFasterWhisperProgress((data) => {
  ...
  const handleRunFasterWhisper = async () => {
    ...
    try {
      const res = await api.runWakuFasterWhisperAlignment?.({
      ...
      if (res?.success && res.jsonContent) {
        ...
        const report = validateWakuWordTranscript(res.jsonContent);
  ...
  const loadTopicVoData = async (topicId: number) => {
    ...
    if (savedMergedTranscriptJson) {
      try {
        const report = validateWakuWordTranscript(savedMergedTranscriptJson);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        console.error('Error initializing Waku 2-Part VO step:', err);
      }
    })();
  }, []);
  ...
  const processPartUpload = async (file: File, partId: number) => {
    ...
    if (api?.uploadWakuVoAudio) {
      res = await api.uploadWakuVoAudio(partId, (file as any).path || file.name, bufferArray, activeTopicId || undefined);
    } else {
  ...
  const processMergedDirectUpload = async (file: File) => {
    ...
    let res: any = null;
    if (api?.uploadWakuVoAudio) {
      res = await api.uploadWakuVoAudio(undefined, (file as any).path || file.name, bufferArray, activeTopicId || undefined);
    } else {
      const objectUrl = URL.createObjectURL(file);
  ...
  const handleRunMergeAudioPipeline = async (customParts?: VoPartItem[]) => {
    ...
    try {
      if (!api?.mergeWakuVoAudio) {
        throw new Error('API mergeWakuVoAudio tidak tersedia di environment ini.');
      }

      const mergeRes = await api.mergeWakuVoAudio(audioPaths, activeTopicId || undefined);
  ...
  const handleProcessManualTranscriptJson = (targetKey: string | number) => {
    ...
    const report: WakuTranscriptValidationReport = validateWakuWordTranscript(jsonText);
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-base font-extrabold text-white">
                Audio Narasi Waku & Gemini AI Audio Alignment (Topik #{activeTopicId || 1})
              </h2>
            </div>
          ...
          >
            ...
            <div className="space-y-1">
              <h5 className="text-xs font-bold text-white">
                Drag & Drop atau Klik untuk Upload 1 File Audio Narasi Waku (Topik #{activeTopicId || 1})
              </h5>
...
export default WakuVoiceOverStep;
