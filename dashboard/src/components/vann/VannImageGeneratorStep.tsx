        ...
        if (Array.isArray(parsedGen.images)) {
          parsedGen.images.forEach((img: any) => {
            if (img && img.segment_id) {
              ...
              if (isForThisTopic && (img.url || img.filePath || img.status === 'error' || img.error)) {
    ...
    if (parsedPrompts.length > 0) {
      const mappedItems: GeneratedImageItem[] = parsedPrompts.map((p) => {
        ...
        return {
          ...
          status: existing?.status === 'error' ? ('error' as const) : existing?.url ? ('success' as const) : ('idle' as const),
      ...
      <div className="bg-gradient-to-r from-emerald-950/80 via-gray-900 to-gray-950 p-6 rounded-3xl border border-emerald-800/40 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            ...
            <div className="flex items-center gap-2 bg-gray-950/90 px-3 py-2 rounded-xl border border-emerald-800/60 text-xs shadow-inner">
              <span className="text-gray-300 font-semibold text-[11px] flex items-center gap-1">
              ...
              <select
                ...
                className="bg-gray-900 text-emerald-300 font-mono font-bold px-2.5 py-1 rounded-lg border border-emerald-700/60 focus:outline-none focus:border-emerald-400 text-xs cursor-pointer hover:bg-gray-800 transition-all"
              >
                <option value={1}>1 Slot (Stable / Low RAM)</option>
                <option value={2}>2 Slots</option>
                <option value={3}>3 Slots</option>
                <option value={4}>4 Slots</option>
                <option value={5}>5 Slots (Default)</option>
                <option value={8}>8 Slots (Fast)</option>
                <option value={10}>10 Slots (Max Speed)</option>
              </select>
            </div>

            {/* Keep Chromium Open Checkbox */}
            <label className="flex items-center gap-2 bg-gray-950/90 px-3 py-2 rounded-xl border border-emerald-800/60 text-xs cursor-pointer hover:border-emerald-600/80 transition-all select-none shadow-inner">
              <input
                type="checkbox"
                checked={keepOpen}
                onChange={(e) => setKeepOpen(e.target.checked)}
                className="w-4 h-4 rounded border-gray-700 bg-gray-900 text-emerald-500 focus:ring-emerald-500/30 accent-emerald-500 cursor-pointer"
              />
              <span className="text-gray-200 font-bold text-[11px] flex items-center gap-1">
                <span>👁️</span> Jangan Tutup Chromium
              </span>
            </label>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 border-b border-gray-800/80 pb-2.5">
                      <div className="space-y-1">
                        ...
                        {item.error && (
                          <div className="ml-8 mt-1.5 p-2.5 bg-red-950/60 border border-red-800/80 rounded-xl text-red-300 text-xs font-mono flex items-center gap-2">
                            <span>⚠️</span>
