// dashboard/src/components/spensia/SpensiaImagePromptStep.tsx
import React, { useState, useEffect } from 'react';
import { validateSpensiaImagePrompts, SpensiaImagePromptItem, SpensiaImagePromptsValidationReport } from '../../utils/spensiaValidation';
...
const SpensiaImagePromptStep: React.FC = () => {
  ...
  const [imagePrompts, setImagePrompts] = useState<SpensiaImagePromptItem[]>([]);
  const [activeTab, setActiveTab] = useState<'cards' | 'json'>('cards');

  const [toast, setToast] = useState<string | null>(null);
  const [validationReport, setValidationReport] = useState<SpensiaImagePromptsValidationReport | null>(null);
  ...
  const loadPromptFromFile = async () => {
    try {
      if (api?.readFromProject) {
        const loadedPrompt = await api.readFromProject('dashboard/prompts/spensia/image-prompt-generator-prompt.md');
  ...
  const handleSavePrompt = async () => {
    try {
      if (api?.saveToProject) {
        await api.saveToProject('dashboard/prompts/spensia/image-prompt-generator-prompt.md', masterPrompt);
  ...
  const handleProcessOutput = async () => {
    ...
    const report = validateSpensiaImagePrompts(pastedOutput);
    setValidationReport(report);
  ...
  const handleCopySinglePrompt = async (item: SpensiaImagePromptItem) => {
    if (api?.copyToClipboard) {
                  ...
                  <div className="bg-gray-950 border border-dashed border-gray-800 rounded-3xl p-12 text-center space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-gray-500 max-w-sm mx-auto">
                        Klik <strong>"Auto Generate Prompts (AI)"</strong> untuk membuat prompt gambar ber-style Spensia DNA.
                      </p>
                    </div>
                  </div>
                ) : (
...
export default SpensiaImagePromptStep;
