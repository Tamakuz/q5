...
const Sidebar: React.FC<SidebarProps> = ({ activeStep, onStepChange, contentMode, onModeChange }) => {
  const activeStepsList = contentMode === 'shortform' ? SHORTFORM_STEPS : LONGFORM_STEPS;
    ...
    <nav className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col py-4 shrink-0 justify-between">
      <div className="space-y-4">
        ...
        <div className="space-y-1">
          <div className="px-4 pb-1.5 flex items-center justify-between text-[11px] text-gray-500 font-mono">
            ...
            <span className={contentMode === 'shortform' ? 'text-indigo-400 font-bold' : 'text-purple-400 font-bold'}>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className={`text-[10px] block font-normal truncate mt-0.5 ${
                    isActive ? 'text-purple-100 opacity-90' : 'text-gray-500 group-hover:text-gray-400'
                  }`}>
                    {step.subText}
                  </span>
                </div>
      ...
      <div className="px-4 pt-3 border-t border-gray-800/80">
        <div className="bg-gray-950 p-3 rounded-xl border border-gray-800/80 space-y-1">
          <span className="text-[10px] text-gray-500 block uppercase font-bold tracking-wider">ACTIVE WORKFLOW MODE</span>
          <span className={`text-xs font-bold block ${contentMode === 'shortform' ? 'text-indigo-400' : 'text-purple-400'}`}>
            {contentMode === 'shortform' ? '📱 Shorts / TikTok (9:16)' : '🍿 Alur Cerita Film (16:9)'}
          </span>
        </div>
      </div>
    </nav>
