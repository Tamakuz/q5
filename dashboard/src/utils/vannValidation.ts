...
export function extractCleanJsonString(raw: string): string {
  ...
  // 2. Extract strictly matching pair based on first opening symbol
  ...
  if (firstObj !== -1 && (firstArr === -1 || firstObj < firstArr)) {
    ...
    if (lastObj > firstObj) {
      cleaned = cleaned.substring(firstObj, lastObj + 1).trim();
    }
  } else if (firstArr !== -1) {
    const lastArr = cleaned.lastIndexOf(']');
    if (lastArr > firstArr) {
      cleaned = cleaned.substring(firstArr, lastArr + 1).trim();
    }
  }
